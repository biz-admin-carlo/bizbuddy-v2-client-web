...
export const exportEmployeePunchLogsCSV = async ({
  ...
  try {
    ...
    // Filter columns to only visible ones (excluding 'actions'), always append lunch window cols
    const exportColumns = [
...
export const exportEmployeePunchLogsPDF = async ({
  ...
  try {
    ...
    // Select appropriate columns for PDF (limit for space), always append lunch window cols
    const pdfColumns = [
...
export const exportEmployeePunchLogsCSV_v2 = async ({
  ...
  try {
    ...
    const fmtGridBreakTime = (d) =>
      d ? new Date(d).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true, timeZone: companyTimezone }) : "";

    // ── Row 1: date number header — date centered above AM/Regular/PM/Lunch ──
    const dateRow = ["", "", "", ""];
    for (const date of dates) {
      const dayNum = new Date(date + "T00:00:00").getDate();
      dateRow.push("", dayNum, "", "");  // blank AM, date above Regular, blank PM, blank Lunch
    }
    ...
    for (let i = 0; i < dates.length; i++) {
      colRow.push("Driver Aide AM", "Regular", "Driver Aide PM", "Lunch Break");
    }
    colRow.push("Driver Aide AM", "Regular", "Driver Aide PM", "", "", "", "", "SL - Sick Leave");

    // ── Employee data rows ────────────────────────────────────────────────────
    const dataRows = [];
    const grandTotals = { am: 0, reg: 0, pm: 0 };
    ...
    for (const [, entry] of sortedEmployees) {
      ...
      for (const date of dates) {
        ...
        totalReg += regH;
        totalPM  += pmH;
        const lunchStart = fmtGridBreakTime(log.lunchBreak?.start);
        const lunchEnd   = fmtGridBreakTime(log.lunchBreak?.end);
        const lunchWindow = lunchStart && lunchEnd ? `${lunchStart} – ${lunchEnd}` : "";
        row.push(
          amH  > 0 ? amH.toFixed(2)  : "",
    ...
    const grandTotal = grandTotals.am + grandTotals.reg + grandTotals.pm;
    totalRow.push(
