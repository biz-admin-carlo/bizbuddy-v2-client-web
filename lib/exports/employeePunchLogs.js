...
export const exportEmployeePunchLogsCSV = async ({
  ...
  try {
    ...
    // Filter columns to only visible ones (excluding 'actions'); lunch window is BNC-only
    const exportColumns = [
      ...visibleColumns.filter(col => col !== "actions"),
      ...(!isDayCare ? ["lunchStart", "lunchEnd"] : []),
    ];
...
export const exportEmployeePunchLogsPDF = async ({
  ...
  try {
    ...
    // Select appropriate columns for PDF (limit for space); lunch window is BNC-only
    const pdfColumns = [
