...
// Updated mappings for better day display
...
// Enhanced day name mapping for better display
const DAY_DISPLAY = {
  MO: "Mon",
  TU: "Tue", 
  WE: "Wed",
...
// Enhanced parseRRule function to handle various formats
...
const DateTimeDisplayWithTooltip = ({ dateTime, type }) => {
  const getTimeDefinition = (type) => {
    switch (type) {
      case 'created':
        return "When this schedule was first created in the system";
      case 'updated':
        return "When this schedule was last modified or updated";
      default:
        return "";
    }
  };
                        ...
                        {columnVisibility.includes("days") && (
                          <TableCell>
                            <div className="flex flex-wrap gap-1 justify-center">
                              {parseRRule(s.recurrencePattern).map((d, index) => (
                                <Badge key={`${d}-${index}`} variant="outline">
                                  {DAY_DISPLAY[d] || d}
                                </Badge>
                              ))}
                            </div>
