/* Modernized Schedule.jsx - Employee Dashboard */
"use client";
...
// Enhanced Shift Card Component
function ShiftCard({ shift, index, compact = false }) {
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center text-xs text-gray-600 dark:text-gray-400">
          ...
          <span className="font-mono">{safeTimeFormat(startTime)}</span>
          <ArrowRight className="h-3 w-3 mx-2" />
          <span className="font-mono">{safeTimeFormat(endTime)}</span>
        </div>
      </motion.div>
    );
  }
                ...
                {loading ? (
                  ...
                  <div className="space-y-3">
                    {shiftsToday.map((shift, index) => (
                      <ShiftCard key={shift.id} shift={shift} index={index} compact />
                    ))}
                  </div>
                ) : (
